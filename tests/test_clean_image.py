...
JPEG_1X1 = base64.b64decode(
    ""
)
...
def _jpeg_comments(data: bytes) -> list[bytes]:
    ...
    while pos + 4 <= len(data):
        ...
        if marker == 0xFE:
            comments.append(data[pos + 2 : pos + length])
        pos += length
    return comments
...
def test_keep_mode_preserves_benign_jpeg_comments_roundtrip(tmp_path: Path):
    comments = (
    ...
    for index, comment in enumerate(comments):
        src = tmp_path / f"comment-{index}.jpg"
        dest = tmp_path / f"comment-{index}.cleaned.jpg"
        src.write_bytes(_jpeg_with_comment(comment))

        before = _run("inspect_file.py", str(src), "--json")
        assert before.returncode == 0
        assert json.loads(before.stdout)["has_ai_metadata"] is False
        ...
        assert _jpeg_comments(dest.read_bytes()) == [comment]

        after = _run("inspect_file.py", str(dest), "--json")
        assert after.returncode == 0
        assert json.loads(after.stdout)["has_ai_metadata"] is False


def test_keep_mode_drops_ai_and_c2pa_jpeg_comments_roundtrip(tmp_path: Path):
    comments = (
        (b"Generated by AI with OpenAI", False),
        (b"contentcredentials c2pa manifest note", True),
    )
    for index, (comment, has_c2pa) in enumerate(comments):
        src = tmp_path / f"marked-comment-{index}.jpg"
        dest = tmp_path / f"marked-comment-{index}.cleaned.jpg"
        src.write_bytes(_jpeg_with_comment(comment))

        before = _run("inspect_file.py", str(src), "--json")
        assert before.returncode == 1
        before_report = json.loads(before.stdout)
        assert before_report["has_ai_metadata"] is True
        assert before_report["has_c2pa"] is has_c2pa
        ...
        assert cleaned.returncode == 0
        assert _jpeg_comments(dest.read_bytes()) == []
